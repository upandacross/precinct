...
class FlippableDVARebuilder:
    ...
    def get_governor_votes_lookup(self, conn):
        """Create a comprehensive governor votes lookup."""
        print("🏛️  Building governor votes lookup for all precincts...")
        
        # Create temporary table for governor lookup
        conn.execute(text("""
    ...
    def process_all_operations(self, dry_run=False, backup_existing=False):
        ...
        with self.engine.connect() as conn:
            # Step 1: Build governor lookup
            self.get_governor_votes_lookup(conn)
            
            # Step 2: Find DVA viable races
            race_count = self.find_dva_viable_races(conn)
            
            # Step 3: Rebuild flippable table
            self.rebuild_flippable_table(conn, dry_run=dry_run)
            
            # Step 4: Show precinct 74 results
            if not dry_run:
                self.show_precinct_74_results(conn)
            
            return race_count
    
    def find_dva_viable_races(self, conn):
    ...
    def rebuild_flippable_table(self, dry_run=False):
        """Rebuild the flippable table with DVA criteria."""
    ...
    def cleanup_temp_tables(self):
        """Clean up temporary tables created during rebuild."""
        print("🧹 Cleaning up temporary tables...")
        
        temp_tables = ['temp_dva_races', 'temp_governor_lookup']
    ...
    def show_precinct_74_results(self):
...
def main():
    ...
    try:
        ...
        # Step 3: Build governor lookup
        rebuilder.get_governor_votes_lookup()
        
        # Step 4: Find DVA viable races
        race_count = rebuilder.find_dva_viable_races()
        
        # Step 5: Rebuild flippable table
        rebuilder.rebuild_flippable_table(dry_run=args.dry_run)
        
        # Step 6: Show precinct 74 results
        if not args.dry_run:
            rebuilder.show_precinct_74_results()
